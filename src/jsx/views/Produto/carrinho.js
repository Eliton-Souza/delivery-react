...
import { api, getCarrinho, limparCarrinho, setCarrinhoCompras } from '../../../services/api';
import { atualizaSubtotal, calculaTotalNova, carrinhoVazio } from './helper';



const Carrinho = ({modal, setModal}) =>{

    const [carrinho, setCarrinho] = useState(JSON.parse(getCarrinho()));
    ...
    const atualizarQuantidade = (index, quantidade) => {
        // Encontre o índice do produto no array carrinho.itens
       // const index = carrinho.itens.findIndex(item => item.id_produto === id_produto);
        ...
        if (index !== -1) {
            ...
            if (carrinho.itens[index].quantidade >= 50) {
                carrinho.itens[index].quantidade=50;
            }
    
            // Verifique se a quantidade é maior que 0
            if (carrinho.itens[index].quantidade <= 0) {
                // Remova o produto do carrinho se a quantidade for menor ou igual a zero
                carrinho.itens.splice(index, 1);
            }else{
                atualizaSubtotal(carrinho.itens[index]);
            }
            
            if(!carrinhoVazio(carrinho)){
                calculaTotalNova(carrinho)
                setCarrinhoCompras(carrinho);                   // Atualize o localStorage com o carrinho atualizado
                setCarrinho(JSON.parse(getCarrinho()));	
            }
            else{
                setCarrinho(null);
            }
            ...
            swal("Oops", "Erro ao fazer alteração", "error");
            limparCarrinho();
        }
    }
        ...
        <Modal.Header closeButton>
            <Modal.Title>
                <div className="card-header border-0 pb-0">
                    <h4 className="cate-title">Seu Carrinho</h4>
                </div>
            </Modal.Title>
        </Modal.Header>
        <Modal.Body>
            ...
            <div className="col-xl-12">
                <div className="card dlab-bg dlab-position">
                    ...
                    <div className="card-body pt-0 pb-2">
                        <div className="row">
                            <p className="font-w500 text-primary fs-15 mt-2">Seu endereço</p>
                            ...
                            <div className="order-check d-flex align-items-center my-3" key={index}>
                                ...
                                <div className="dlab-info">
                                    <div className="d-flex align-items-center justify-content-between">
                                        <h4 className="dlab-title"><Link to={"#"}>{item.tipo !== "fixo" ? item.nome +" "+ item.tamanho : item.nome}</Link></h4>
                                        <h4 className="text-primary ms-2">R$ {item.subtotal}</h4>
                                    </div>
                                    <div className="d-flex align-items-center justify-content-between">
                                        <div className="d-flex">
                                            {item.tipo !== "fixo" && (
                                                <div className="d-flex flex-column">
                                                    {item.sabores.map((sabor, ind) => (
                                                        <div key={ind} className="d-flex align-items-center me-3">
                                        ...
                                        <div className="quntity" style={{alignSelf: 'flex-start'}}>
                                            <button data-decrease onClick={()=> atualizarQuantidade(index, -1)}>-</button>
                                            <input data-value type="text" value={item.quantidade}/>
                                            <button data-increase onClick={()=> atualizarQuantidade(index, 1)}>+</button>
                                        </div>
                                    </div>             
                        ...
                        {!carrinho && (
                            <div className="order-check d-flex align-items-center my-3">
                                <div className="d-flex align-items-center justify-content-between">
                                    <h4 className="dlab-title">Carrinho Vazio</h4>
                                </div>                                   
                            </div>
                        )}                        
                    </div>	
                    ...
                    {carrinho && (
                        <div className="card-footer  pt-0 border-0">
                            ...
                            <div className="d-flex align-items-center justify-content-between">
                                <p>Serviço</p>
                                <h4 className="font-w500">R$ 1,00</h4>
                            </div>
                            <div className="d-flex align-items-center justify-content-between">
                                <p>Entrega</p>
                                <h4 className="font-w500">R$ 0,00</h4>
                            </div>
