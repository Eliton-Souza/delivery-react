...
import { atualizaSubtotal, calcularTotal } from './helper';
...
const Carrinho = () =>{
    ...
    const atualizarQuantidade = (index, quantidade) => {
        ...
        carrinhoAtual.itens[index].quantidade += quantidade;

        if (carrinhoAtual.itens[index].quantidade >= 50) {
            carrinhoAtual.itens[index].quantidade=50;
        }

        if (carrinhoAtual.itens[index].quantidade <= 0) {
            // Remova o produto do carrinho se a quantidade for menor ou igual a zero
            carrinhoAtual.itens.splice(index, 1);
        }else{
            carrinhoAtual.itens[index].subtotal= atualizaSubtotal(carrinhoAtual.itens[index]);
        }
                    
        if(carrinhoAtual.itens.length > 0){
            carrinhoAtual.total= calcularTotal(carrinhoAtual.itens);
